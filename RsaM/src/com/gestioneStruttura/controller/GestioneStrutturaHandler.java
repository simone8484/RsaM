...
public class GestioneStrutturaHandler extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (action.equals("selezionaResponsabileReparto")){
			...
			ServiceRegistry servReg2 = new StandardServiceRegistryBuilder().applySettings(config2.getProperties()).build();
			SessionFactory factory2 = config2.buildSessionFactory(servReg2);
			Session session2 = factory2.openSession();
			session2.beginTransaction();
			
			repartoCorrente.setCodiceFiscaleResponsabileReparto(request.getParameter("responsabileReparto"));
			session2.saveOrUpdate(repartoCorrente);
